...
type OrderBook struct {
	orders map[int64]*list.Element // orderID -> *Order (*list.Element.Value.(*Order))
...
// NewOrderBook creates Orderbook object
func GetOrderBookByID(id string) *OrderBook {
...
func (ob *OrderBook) ProcessMarketOrderV1(orderID int64, side Side, quantity decimal.Decimal) (matchedLog []*MatchedLog, partial *Order, partialQuantityProcessed, quantityLeft decimal.Decimal, err error) {
	if quantity.Sign() <= 0 {
		return nil, nil, decimal.Zero, decimal.Zero, ErrInvalidQuantity
	}
	...
	for _, order := range done {
		matchedLog = append(matchedLog, &MatchedLog{
	...
	if partial != nil {
		matchedLog = append(matchedLog, &MatchedLog{
			MakerOrderID: partial.id,
			TakerOrderID: orderID,
			Quantity:     partialQuantityProcessed,
			Price:        partial.price,
			Time:         now,
			Side:         partial.side,
		})
...
//	partialQuantityProcessed - if partial order is not nil this result contains processed quatity from partial order
func (ob *OrderBook) ProcessLimitOrder(side Side, orderID int64, quantity, price decimal.Decimal) (done []*Order, partial *Order, partialQuantityProcessed decimal.Decimal, err error) {
...
func (ob *OrderBook) ProcessLimitOrderV1(side Side, orderID int64, quantity, price decimal.Decimal) (matchedLog []*MatchedLog, partial *Order, partialQuantityProcessed decimal.Decimal, err error) {
	if _, ok := ob.orders[orderID]; ok {
		return nil, nil, decimal.Zero, ErrOrderExists
	}
...
func (ob *OrderBook) Order(orderID int64) *Order {
...
// Depth returns price levels and volume at price level with limit return
func (ob *OrderBook) DepthWithLimit(limit int) (asks, bids [][]decimal.Decimal) {
	level := ob.asks.MaxPriceQueue()
	limitAsks := 0
	limitBids := 0
	asks = [][]decimal.Decimal{}
	bids = [][]decimal.Decimal{}
	for level != nil && limitAsks < limit {
		asks = append(asks, []decimal.Decimal{level.price, level.volume})
		level = ob.asks.LessThan(level.Price())
		limitAsks++
	}

	level = ob.bids.MaxPriceQueue()
	for level != nil && limitBids < limit {
		bids = append(bids, []decimal.Decimal{level.price, level.volume})
		level = ob.bids.LessThan(level.Price())
		limitBids++
	}
	return
...
func (ob *OrderBook) UnmarshalJSON(data []byte) error {
	...
	ob.asks = obj.Asks
	ob.bids = obj.Bids
	ob.orders = map[int64]*list.Element{}
